...
def InitFig(NumSubplots):
    """
    Initialize a figure with a specified number of subplots.
...
def LivePlot(X, Y):
    """
    Update the current figure with data. If there is no existing figure, create it.

    Parameters:
    X (list of arrays): X data. Would be the bin centers for an histogram
    Y (list of arrays): Y data.
    """
